...
import QtQml
import Qt.labs.platform as QLab
// Qaterial
...
Item
{
  ...
  Component
  {
	...
	Qaterial.AlertIconDialog
	{
	  ...
	  standardButtons: root.settings && root.settings.standardButtons ? root.settings.standardButtons : DialogButtonBox.NoButton

	  /******** VJP *******/
	  iconFill: root.settings ? root.settings.iconFill : true
	  ...
	  {
		if(root.settings && root.settings.onNo)
		  root.settings.onNo()
	  }
	} // AlertIconDialog
  } // Component
  ...
  Component
  {
	...
	Qaterial.RadioDialog
	{
	  ...
	  {
		  if(root.settings && root.settings.onAccepted)
			root.settings.onAccepted(currentIndex)
  ...
  Component
  {
	...
	Qaterial.ListDialog
	{
	  ...
	  Component.onCompleted:
	  {
		...
		if(!model)
		  console.log("Error : ListDialog : model is null on open")
	  }
	} // RadioDialog
  } // Component
  ...
  Component
  {
	id: _checkListViewComp

	Qaterial.CheckDialog
	{
	  title: (root.settings && root.settings.title) ? root.settings.title : ""
	  currentIndex: (root.settings && root.settings.currentIndex !== undefined) ? root.settings.currentIndex : -1
	  ...
	  {
		if(root.settings && root.settings.onAccepted)
			root.settings.onAccepted(selectedIndices)
	  }
	  ...
	  Component.onCompleted:
	  {
		...
		if(!model)
		  console.log("Error : CheckListViewDialog : model is null on open")
	  }
	} // RadioDialog
  } // Component
  ...
  function openListView(settings)
  {
	show(settings, _simpleListViewComp)
  }

  function openCheckListView(settings)
  {
	show(settings, _checkListViewComp)
  }
